<template>
    ...
    <span v-if="visible">
        ...
        <el-dialog
            ...
            class="iBrain-table-select-dialog"
            :title="title"
        ...
        >
            <el-form
                    class="iBrain-table-select-form"
                    @submit.native.prevent
                    :label-width="(labelWidth || '100px')"
                    v-if="queryData.length"
            >
                <el-row>
                    <el-col :span="(item.collspan || queryCollspan)" v-for="item in queryData" :key="item.prop">
                        <el-form-item :label="item.label">
                            <iBrain-select
                                v-if="item.querySelect && item.querySelect.length"
                                v-model="params[item.prop]"
                                :options="item.querySelect"
                                :valueKey="item.valueKey"
                                :labelKey="item.labelKey"
                                @clear="onSelectClear(item)"
                                clearable
                            />
                            <el-input
                                v-else
...
export default {
    ...
    props: {
        ...
        inputKey: {
            type: String, // input 对应字段名称 不传取查询条件第一条 或 列数据第一条
            default: ''
        },
        columns: {
            type: Array,
    ...
    computed: {
        ...
        btOffset () {
            ...
            this.queryData.forEach((item) => {
                collspan += Number(item.collspan || this.queryCollspan)
                if (collspan > 24) {
                    collspan = item.collspan || this.queryCollspan
                }
            })
    ...
    methods: {
        ...
        //     let inputKey = this.getInputKey()
        //     this.tableList.map(item => {
        //         if (item[inputKey] == this.innerValue) {
        //             this.$table.selectRow(item)
        //         }
        //     })
        // },
        initParams () {
        ...
        onFocus() {
            this.$input.blur();
            if (this.readonlyTips) {
                this.$message.warning(this.readonlyTips)
                return
            }
        ...
        getInputKey () {
            if (this.inputKey) return this.inputKey
            if (this.queryData.length){
                return this.queryData[0].prop
            }
            return this.columns.length ? this.columns[0].prop : ''
        },
        emitChangeValue() {
            if (!this.innerValue) {
        ...
        onSubmit() {
            let inputKey = this.getInputKey()
            this.innerValue = this.tableSelectList.map((item) => item[inputKey]).join(',')
            this.emitChangeValue()
