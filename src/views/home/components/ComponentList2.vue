<template>
  <div class="container">
    <div class="main">
      <draggable :list="comlist2.length > 0 ? comlist2.slice(4, 7) : componentList.slice(4, 7)" class="main"
        :group="{ name: 'componentsGroup1', pull: 'clone', put: false, }" :sort="true" :clone="cloneComponent"
        draggable=".drag-item" filter='.item' @end="onEnd">
        <div class="drag-item" v-for="item in (comlist2.length > 0 ? comlist2.slice(4, 7) : componentList.slice(4, 7))"
          :key="item.id">
    ...
    onEnd(obj) {
      ...
      if (obj.from !== obj.to) {
        if (obj.to.className.indexOf('row-drag') < 0) {
          ...
          this.page2.widgets.splice(obj.newIndex, 0, this.activeData);
        }
        if (obj.to.className === '') {
          this.$set(this.componentList.slice(4, 7)[obj.oldIndex], 'flag', true)
        }
      }
    }
  }
